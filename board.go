...
func (b *Board) update(m *Move) {
	...
	// Precompute rook movement for Chess960 castling using the pre-move board.
	hasCastle := m.HasTag(KingSideCastle) || m.HasTag(QueenSideCastle)
	castleRFrom := NoSquare
	castleRTo := NoSquare
	kingDest := m.s2
	if hasCastle {
		side := KingSide
		...
		if rFrom, rTo, ok := chess960CastleRookSquares(b, p1.Color(), side); ok {
			castleRFrom, castleRTo = rFrom, rTo
		} else {
			hasCastle = false
		}
	}

	// Handle castling explicitly to support Chess960 cases where the king may remain
	// on its starting square (s1 == s2). The generic move loop would otherwise remove
	// the king when s1 == s2.
	if hasCastle {
		// Move king if destination differs; otherwise ensure it stays on the same square.
		if kingDest != m.s1 {
			bb := b.bbForPiece(p1)
			b.setBBForPiece(p1, (bb & ^s1BB)|s2BB)
		}
		// Move rook involved in castling.
		if castleRFrom != NoSquare && castleRTo != NoSquare {
			rPiece := NewPiece(Rook, p1.Color())
			fromBB := bbForSquare(castleRFrom)
			toBB := bbForSquare(castleRTo)
			rBB := b.bbForPiece(rPiece)
			b.setBBForPiece(rPiece, (rBB&^fromBB)|toBB)
		}
		b.calcConvienceBBs(nil)
		return
	}
	...
	for _, p := range allPieces {
		bb := b.bbForPiece(p)
		// remove what was at s2 unless it's the friendly rook that will castle off this square
		if !(hasCastle && p.Type() == Rook && p.Color() == p1.Color() && m.s2 == castleRFrom) {
	...
	// move rook for castle (Chess960 aware)
	if hasCastle {
		rFrom, rTo := castleRFrom, castleRTo
		if p1.Color() == White {
			b.bbWhiteRook = (b.bbWhiteRook & ^bbForSquare(rFrom)) | bbForSquare(rTo)
		} else {
			b.bbBlackRook = (b.bbBlackRook & ^bbForSquare(rFrom)) | bbForSquare(rTo)
		}
	}
