...
func (UCINotation) Decode(pos *Position, s string) (*Move, error) {
	...
	if p.Type() == King {
		// Chess960: tag castle when king moves onto or past a rook, or the orthodox squares.
		if (s1 == E1 && s2 == G1) || (s1 == E8 && s2 == G8) {
			m.addTag(KingSideCastle)
		} else if (s1 == E1 && s2 == C1) || (s1 == E8 && s2 == C8) {
			m.addTag(QueenSideCastle)
		} else {
			// If destination has a friendly rook on same rank, treat as castle.
			rook := NewPiece(Rook, p.Color())
			if pos.Board().Piece(s2) == rook && s1.Rank() == s2.Rank() {
				if s2.File() > s1.File() {
					m.addTag(KingSideCastle)
					// Normalize destination to standard Chess960 king target (g-file).
					if s1.Rank() == Rank1 {
						s2 = G1
					} else {
						s2 = G8
					...
					m.addTag(QueenSideCastle)
					// Normalize destination to standard Chess960 king target (c-file).
					if s1.Rank() == Rank1 {
						s2 = C1
					} else {
						s2 = C8
...
func (AlgebraicNotation) Encode(pos *Position, m *Move) string {
	// Chess960: if move is tagged as castle, always emit castle SAN even if king stayed on its file.
