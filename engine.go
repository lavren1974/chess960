...
// Chess960 helpers
func chess960CastleRookSquares(b *Board, color Color, side Side) (Square, Square, bool) {
	...
	rookBB := b.bbWhiteRook
	if color == Black {
		rookBB = b.bbBlackRook
	}
	rank := kingSq.Rank()
	targetRook := NoSquare
	for sq := Square(0); sq <= H8; sq++ {
		if !rookBB.Occupied(sq) || sq.Rank() != rank {
			continue
		}
		if side == KingSide {
			if sq.File() > kingSq.File() && (targetRook == NoSquare || sq.File() > targetRook.File()) {
				targetRook = sq
			}
		} else {
			if sq.File() < kingSq.File() && (targetRook == NoSquare || sq.File() < targetRook.File()) {
	...
	return targetRook, rookDest, true
}
...
func isPathClear(b *Board, from, to Square) bool {
	...
	for f := low + 1; f < high; f++ {
		sq := NewSquare(f, from.Rank())
		if !b.emptySqs.Occupied(sq) {
			return false
		}
	}
	return true
...
// pathClearIgnoringRook is like isPathClear but treats a friendly rook on rookSq as passable.
func pathClearIgnoringRook(b *Board, from, to, rookSq Square) bool {
	if from.Rank() != to.Rank() {
		return false
	}
	low := from.File()
...
func kingPathSquares(from, to Square) []Square {
	if from.Rank() != to.Rank() {
		return nil
	}
	var squares []Square
	...
	for f := int(from.File()) + step; f != int(to.File())+step; f += step {
		squares = append(squares, NewSquare(File(f), from.Rank()))
	}
	return squares
}

func chess960CastleMoves(pos *Position) []*Move {
	moves := []*Move{}
	kingSq := pos.board.whiteKingSq
	if pos.turn == Black {
		kingSq = pos.board.blackKingSq
	...
	kingSide := func() bool { _, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), KingSide); return ok }()
	queenSide := func() bool { _, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), QueenSide); return ok }()

	if kingSide {
		rookFrom, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), KingSide)
		path := kingPathSquares(kingSq, targetKingK)
		if ok && isPathClear(pos.board, kingSq, rookFrom) && pathClearIgnoringRook(pos.board, kingSq, targetKingK, rookFrom) && !pos.inCheck && !squaresAreAttacked(pos, path...) {
			m := &Move{s1: kingSq, s2: targetKingK}
	...
	if queenSide {
		rookFrom, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), QueenSide)
		path := kingPathSquares(kingSq, targetKingQ)
		if ok && isPathClear(pos.board, kingSq, rookFrom) && pathClearIgnoringRook(pos.board, kingSq, targetKingQ, rookFrom) && !pos.inCheck && !squaresAreAttacked(pos, path...) {
			m := &Move{s1: kingSq, s2: targetKingQ}
...
func castleMoves(pos *Position) []*Move {
	moves := []*Move{}
	// Chess960: derive availability from current board instead of castleRights,
	// since rights tracking may be missing for non-orthodox starts.
	kingSide := func() bool { _, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), KingSide); return ok }()
	queenSide := func() bool { _, _, ok := chess960CastleRookSquares(pos.board, pos.Turn(), QueenSide); return ok }()
	if kingSide || queenSide {
		return append(moves, chess960CastleMoves(pos)...)
	}
	return moves
}
